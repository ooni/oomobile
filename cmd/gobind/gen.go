...
import (
	...
	"github.com/ooni/oomobile/bind"
	"github.com/ooni/oomobile/internal/importers"
	"github.com/ooni/oomobile/internal/importers/java"
	"github.com/ooni/oomobile/internal/importers/objc"
...
func genPkg(lang string, p *types.Package, astFiles []*ast.File, allPkg []*types.Package, classes []*java.Class, otypes []*objc.Named) {
	...
	case "java":
		...
		if p == nil {
			dir, err := packageDir("github.com/ooni/oomobile/bind")
			if err != nil {
				errorf(`"github.com/ooni/oomobile/bind" is not found; run go get github.com/ooni/oomobile/bind: %v`, err)
				return
			}
			repo := filepath.Clean(filepath.Join(dir, "..")) // github.com/ooni/oomobile directory.
			...
			javaDir, err := packageDir("github.com/ooni/oomobile/bind/java")
			if err != nil {
				errorf("unable to import bind/java: %v", err)
				return
			}
	...
	case "objc":
		...
		if p == nil {
			// Copy support files
			dir, err := packageDir("github.com/ooni/oomobile/bind/objc")
			if err != nil {
