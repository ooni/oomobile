...
func runBuildImpl(cmd *command) (*packages.Package, error) {
	...
	if !nmpkgs["github.com/ooni/oomobile/app"] {
		return nil, fmt.Errorf(`%s does not import "github.com/ooni/oomobile/app"`, pkg.PkgPath)
	}
...
func extractPkgs(nm string, path string) (map[string]bool, error) {
	if buildN {
		return map[string]bool{"github.com/ooni/oomobile/app": true}, nil
	}
	r, w := io.Pipe()
